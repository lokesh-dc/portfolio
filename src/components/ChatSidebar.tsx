...
import { AboutResponse, ExperienceResponse, ProjectsResponse } from "./Responses";
...
export default function ChatSidebar({ isOpen, onClose }: ChatSidebarProps) {
      ...
      content: (
        <div className="space-y-4">
          ...
          <p className="text-stone-600 dark:text-stone-400">
            How can I help you learn more today? Feel free to use the prompts below.
          </p>
        </div>
      ),
    },
  ...
  const handleOptionSelect = (option: "experience" | "projects" | "about") => {
    ...
    const userMsg: Message = {
      id: generateId(),
      type: "user",
      content: <p className="text-stone-800 dark:text-stone-200 font-medium">{userText}</p>,
    };
        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto scroll-smooth pb-4">
              {messages.map((msg) => (
                <div key={msg.id} className="border-b border-stone-100 dark:border-stone-800/30 last:border-b-0">
                   <MessageBubble message={msg} />
                </div>
              ))}
              {isTyping && (
                <div className="flex items-center gap-1.5 px-6 py-6 md:pl-16">
                  <div className="w-1.5 h-1.5 rounded-full bg-stone-300 dark:bg-stone-600 animate-bounce" style={{ animationDelay: '0ms' }} />
                  <div className="w-1.5 h-1.5 rounded-full bg-stone-300 dark:bg-stone-600 animate-bounce" style={{ animationDelay: '150ms' }} />
                  <div className="w-1.5 h-1.5 rounded-full bg-stone-300 dark:bg-stone-600 animate-bounce" style={{ animationDelay: '300ms' }} />
                </div>
              )}
            ...
            <div className="flex-none bg-gradient-to-t from-[#fafafa] via-[#fafafa] to-transparent dark:from-[#0c0c0c] dark:via-[#0c0c0c] pt-6 relative z-10">
              <ChatInput onOptionSelect={handleOptionSelect} disabled={isTyping} />
            </div>
          </motion.aside>
        </>
      )}
